...
《铁砧工艺》方块最密堆积计算器。它为以下结构按各自衰变规则计算布局，并尽量减少昂贵的隔离材料：
...
- 邻接只计算六个正交方向。虚空物质块有五个或更多虚空物质块邻面时会衰变。
- 钚块要求相邻钚块数减相邻铅块数不大于 `2`；结构外和集热器不计入钚块数或铅块数。
- 三种模式都只计算并展示一个 `5 x 5 x 5` 单元，不需要输入堆积数量。
- 不堆积模式按独立边界求解；平面模式生成可沿 X/Z 复制的周期单元；立体模式生成可沿 X/Y/Z 复制的周期单元。
- 虚空场景已证明最优的每单元结果依次为：不堆积 `110/14/1`、平面 `93/31/1`、立体 `90/34/1`（主材料/隔离材料/设备）。
- 钚场景当前可行结果依次为：不堆积 `88/36/1`、平面 `86/38/1`、立体 `90/34/1`；铅块已证明下界分别为 `33/35/33`，界面显示为“可行方案”和 `33–36`、`35–38`、`33–34`。
- 每个预计算单元在 Worker 中由独立验证器复核。只有上下界一致时才显示“已证明最优”。

游戏规则版本元信息目前记录为 Minecraft 1.21.1 / AnvilCraft 1.6 开发分支。发布到其他版本前，需要分别复核虚空物质块和钚块规则。
